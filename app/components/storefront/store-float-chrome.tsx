...
export function StoreFloatChrome() {
  ...
  return (
    <>
      {/* Left edge — Bulk Order (hide on very small screens to save space) */}
      <Link
        href="/contact"
          ...
          <div className="bottom-marquee flex w-max items-center gap-3 px-3">
            {row.map((item, index) => (
              <Link
                ...
                href={item.href}
                className="inline-flex shrink-0 items-center gap-2 rounded-full bg-store-navy px-4 py-2 text-xs font-semibold whitespace-nowrap text-white transition hover:bg-store-navy-dark sm:text-sm"
              >
                {item.label}
